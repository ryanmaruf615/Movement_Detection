...
def detect_encoding(filepath):
    """Try multiple encodings and return the one that works."""
...
def load_new_csv(filepath):
    """Load a CSV file - auto-detects encoding and clean/split format."""
    ...
    if col_count >= 50000:
        ...
        return _parse_clean(filepath, encoding)
    else:
        print("  Format: Split lines (reassembling scans...)")
        return _parse_split(filepath, encoding)


def _parse_clean(filepath, encoding):
...
def _parse_split(filepath, encoding):
